...
[`dsh-base`](../packages/bundle/base/README.zh.md) 是 `web`、`headless`、`sdk`、`acp` 与 `tui` profile 的共享第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.zh.md)、[`dsh-headless`](../packages/bundle/headless/README.zh.md)、[`dsh-sdk-app`](../packages/bundle/sdk-app/README.zh.md)、[`dsh-acp-app`](../packages/bundle/acp-app/README.zh.md) 与 [`dsh-tui`](../packages/ui/tui/README.zh.md) 叠加这些表层。[`dsh-sdk-minimal`](../packages/bundle/sdk-minimal/README.zh.md) 是刻意保留的例外：一个组合包拥有完整的显式 SDK 配置树，不应用 `dsh-base`。
...
自定义 profile 默认实时重载 patch。随附的 `web` profile 使用实时重载；`headless`、`sdk`、`sdk-minimal`、`acp` 和 `tui` 则只在启动时应用一次所有配置层。
...
所有受支持的 Node 应用都从 `dsh` CLI 与具名 profile 启动。随附应用是 `dsh web`（刻意为 `--profile web` 保留的别名）、`dsh --profile headless`、`dsh --profile tui`、`dsh --profile sdk`、`dsh --profile sdk-minimal` 与 `dsh --profile acp`。TypeScript SDK 会解析其同版本 `dsh` 依赖并选择 `sdk`；自定义插件组合继续由 profile 与有序 patch 文件表达，而不是另一个可执行文件或内联应用树。`sdk-minimal` 是位于同一 launcher 后的仓库自有独立组合包，而不是由调用方提供的 Cordis 配置树。
