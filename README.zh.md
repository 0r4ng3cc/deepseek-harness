...
分支说明：这个 fork 发布的是 `@deepseek-ai/dsh` 和 `xfdsh` 启动器，可以和上游 `dsh` 并存。下面表格概览这个 fork 相对上游的主要改动。
...
| 领域 | 改动 | 结果 |
| --- | --- | --- |
| 包命名空间 | 工作区包从 `@deepseek-ai/dsh-*` 改为 `@deepseek-ai/dsh-*`；vendor 和 native 包保留上游命名空间。 | fork 拥有自己的发布包线。 |
