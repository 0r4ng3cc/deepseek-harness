...
Fork note: this branch publishes `@deepseek-ai/dsh` and the `xfdsh` launcher so it can coexist with upstream `dsh`. The table below summarizes the main fork changes.
...
| --- | --- | --- |
| Package namespace | Workspace packages were rescoped from `@deepseek-ai/dsh-*` to `@deepseek-ai/dsh-*`; vendor and native packages keep their upstream scopes. | The fork owns its own published package line. |
