...
在 `update_goal` 之前调用 `get_goal`，并复制精确的 `goal_id` 与 `revision`；所有调用都互斥，因此模型排序的批次能观察到更早变更及其新 revision。替换值只属于 `edit`；`blocked_reason` 只有在 `blocked` 时才必填，并以稳定代码 `model-reported` 持久化。严格 schema 下的空字符串和零填充值视为省略，而有意义的值仍限定到各自 action。`create_goal` 会忽略 `max_goal_rounds` 并使用 goal 服务默认上限；`update_goal` 的 edit 会把模型给出的上限抬到该默认值，避免模型随手填 3 或 8 把续行停掉。
