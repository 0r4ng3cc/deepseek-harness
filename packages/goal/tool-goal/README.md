...
Call `get_goal` before `update_goal` and copy the exact `goal_id` and `revision`; all calls are exclusive, so a model-ordered batch observes earlier mutations and their new revisions. Replacements belong only to `edit`; `blocked_reason` is required only for `blocked` and is persisted with the stable code `model-reported`. Strict-schema empty-string and zero fillers count as omitted, while meaningful values remain limited to their action. `create_goal` ignores `max_goal_rounds` and uses the goal-service default; `update_goal` edit floors a supplied cap to that default so a guessed 3 or 8 cannot stop continuation.
