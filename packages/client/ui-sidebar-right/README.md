...
The layout itself — the split tree, its operations, the drag gestures, the floating panels — belongs to `@x1a0f3n9/dsh-client-ui-dockkit` and stays host-agnostic. This package supplies everything that kit refuses to know: the product's copy, what a tab's `kind` means, which tab a fresh pane is seeded with, where the surface is mounted, and how other plugins reach it.
