...
A contribution is a client-owned command — a host-name collision fails loud. Its description is a resolver read for every candidate request, so it follows the active locale. A decoration adds a bare-invocation popup to an EXISTING host command: the host command keeps its catalog row, its argument claim, and its lifecycle logging, and a decorated name with no host row in the session's directory never fires. The six shipped Host commands use bilingual descriptions when both the command name and canonical English description match; scoped shadows and third-party commands keep their reported copy verbatim. The `verify-host-command-descriptions` gate keeps those Host strings and the Client allowlist aligned. Menu queries fuzzy-match ordered, case-insensitive subsequences of command names; prefixes rank first.
