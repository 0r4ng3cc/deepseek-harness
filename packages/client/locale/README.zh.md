...
这些限制说明本地化在哪些地方需要消费方明确配合。它们是当前包约束，不是任务积压。

- **非 slot 注册表必须保留解析函数**——`LocaleRuntime` 会刷新已订阅的渲染路径，但消费方若存储 `t(...)` 的结果而非解析函数，该文本会保持注册时的语言，直至重新注册。命令注册表会存储说明解析函数，因此其菜单行会跟随语言切换。
