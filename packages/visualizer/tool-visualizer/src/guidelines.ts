import type { WidgetGuidelineModule } from './types.ts'
...
- The host provides readable defaults for controls and owns placement and title. Keep those defaults or restyle controls and the visual surface to suit the subject; avoid a second outer frame or repeated panels.
...
- Give every primary visual a concise accessible name and summary without repeating the host title.`

const MOCKUP_GUIDANCE = `## Mockup
...
const GUIDELINES: Omit<Readonly<Record<WidgetGuidelineModule, string>>, 'mockup'> = Object.freeze({
  diagram: `## Diagram
...
- Call window.dshWidget.sendPrompt(text) only from a user gesture and only when the next step genuinely needs the model. The message enters the conversation immediately, labelled as widget-authored; there is no confirmation step.
...
export function composeGuidelines(
  ...
  const selected = [...new Set(modules)].map(module => module === 'mockup'
    ? MOCKUP_GUIDANCE
    : GUIDELINES[module])
