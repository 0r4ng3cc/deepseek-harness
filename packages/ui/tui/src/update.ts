...
// Re-exported for the bin launcher, which reads the compiled copy at
// lib/types/utils/shellQuote.js.
export { shellQuote }

const PACKAGE_NAME = '@x1a0f3n9/dsh-tui'
...
export async function resolveTuiUpdateTarget(): Promise<TuiUpdateTarget> {
  // This package does not publish dsh-tui-standalone-* GitHub assets; never
  // contact a third-party release host for binary replacement.
  if (isStandaloneRuntime()) {
    return { kind: 'unknown', isStandalone: true }
  }
...
export async function updateTui(
  ...
  if (isStandaloneRuntime()) {
    process.stderr.write(
      'dsh-tui: 便携包运行方式不支持自动更新，请使用 dsh --profile tui 或 dsh plugin --profile <name> update @x1a0f3n9/dsh-tui\n',
    )
...
export async function cliUpdate(profile: string): Promise<number> {
  if (isStandaloneRuntime()) {
    process.stderr.write(
      'dsh-tui: standalone/portable runtime cannot auto-update. Use dsh --profile tui or dsh plugin --profile <name> update @x1a0f3n9/dsh-tui.\n',
    )
    return 1
  }
  const target = await resolveTuiUpdateTarget()
