...
import { assembleContextFor, installModelSelection, type Agent, type AgentHandle, type AgentStatus, type CreateAgentOptions, type ModelSelectionRef } from '@x1a0f3n9/dsh-agent'
import type { CommandRuntime } from '@x1a0f3n9/dsh-commands'
import { isModelInvocable, isUserInvocable, renderSkillContent, type SkillSummary } from '@x1a0f3n9/dsh-skill'
import type { LlmConfigurableProvider, LlmDiscoveredModel, LlmModelInfo, LlmProviderInfo } from '@x1a0f3n9/dsh-llm'
import {
  createUserMessage,
  MessageId,
  ReasoningEffortId,
...
import { SessionId, type SessionEvent, type SessionHeader } from '@x1a0f3n9/dsh-session'
import { renderContextSections, renderPrompt } from '@x1a0f3n9/dsh-system-prompt'
import { loadBaselineInstructions } from '@x1a0f3n9/dsh-agent-instructions'
...
function prepareReplayEvents(events: readonly SessionEvent[]): SessionEvent[] {
  ...
  return events.filter((event) => {
    ...
    // Durable rows the SessionEvent union no longer names: `assistant/chunk`
    // deltas and the packed rows that expand into them. Compare through a
    // widened view — the drop is exactly for data the static type doesn't know.
    const durableType = (event as { type: string }).type
    if (durableType === 'assistant/chunk') {
      // Keep only the in-flight tail (no message sealed after it).
      return lastMessageSeq < 0 || event.seq > lastMessageSeq
    }
    if (
      durableType === 'text-chunks' ||
      durableType === 'reasoning-chunks' ||
      durableType === 'tool-call-chunks'
    ) {
      return false
    }
    return true
...
export function createChannel(
   ...
   *  mirror of rc.8's `EncodedImageAttachment` (`@x1a0f3n9/dsh-attachment/
  ...
  const commandServiceSupportsImages = (service: CommandRuntime): boolean => {
    if (installedMeetsVersion('@x1a0f3n9/dsh-commands', '0.1.0-rc.8')) return true
  ...
  const executeRegistryCommand = async (name: string, rawInput: string): Promise<string | undefined> => {
    ...
    try {
      ...
      const execution = images === undefined
        ...
        // `execute` takes tagged submit attachments; a registry image is the
        // image arm's payload, so tag it on the way in.
  ...
  const state: ChannelState = {
    ...
    async sideQuestion(
      ...
      const request: Record<string, unknown> = {
        ...
        // The system prompt travels as a `system/message` in the derived
        // history; the header no longer carries a `system` field.
        messages,
...
${output}
  ...
  const renderEvent = (event: SessionEvent): void => {
    // `assistant/chunk` deltas stay in the log — the jsonl reader expands the
    // packed chunk rows into them — while the pinned peer's SessionEvent union
    // no longer names the type, so admit it structurally like `goal/change`.
    if ((event as { type: string }).type === 'assistant/chunk') {
      const chunkEvent = event as unknown as DurableAssistantChunk
      if (handledAssistantChunks.has(chunkEvent.seq)) return
      handledAssistantChunks.add(chunkEvent.seq)
      const chunk = chunkEvent.data.chunk
      if (chunk.type === 'text-delta') {
        if (chunk.text) {
          ...
          const key = stepKey(chunkEvent.data.turn, chunkEvent.data.step)
          const row = assistantRowsByStep.get(key) ?? ensureStreaming(chunkEvent.seq)
        ...
        if (chunk.text) {
          const row = ensureReasoning(chunkEvent.seq, chunkEvent.data.turn, chunkEvent.data.step)
          appendTextDelta(row, chunk.text)
        }
      }
      if (isTokenDelta(chunk)) statsFirstToken ??= chunkEvent.time
      const step = tpsStep
      if (
        step !== undefined &&
        step.turn === chunkEvent.data.turn &&
        step.step === chunkEvent.data.step &&
        isTokenDelta(chunk)
      ) {
        step.firstTokenTime ??= chunkEvent.time
        step.outputChars += tokenDeltaChars(chunk)
        const elapsedMs = Math.max(0, chunkEvent.time - step.firstTokenTime)
        if (elapsedMs > 500) {
    ...
    switch (event.type) {
      ...
      case 'request/header': {
        ...
        // conversation's call config (provider/model/effort/sampling).
      ...
      case 'system/message': {
        // The effective system prompt is the latest `system` message in the
        // log (RequestContext.systemPromptUpdate is 'in-history'); it seeds
        // the context bar's system segment.
  ...
  const bindAgent = (): void => {
    ...
    agentSubscriptions = [
      ...
      ctx.on('session/event', (session, event) => {
        ...
        if ((event as { type: string }).type === 'assistant/chunk') state.emitStream()
        else state.emit()
      }),
