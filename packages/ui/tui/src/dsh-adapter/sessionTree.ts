 ...
 *
 * @module @x1a0f3n9/dsh-tui/sessionTree
 */
import type { StreamChunk } from '@x1a0f3n9/dsh-llm'
import type { SessionEvent } from '@x1a0f3n9/dsh-session'

/**
 * A durable log row the current `SessionEvent` union no longer names.
 * `assistant/chunk` deltas stay in the log — the jsonl reader expands the
 * packed `text-chunks`/`reasoning-chunks`/`tool-call-chunks` rows into them —
 * so replay meets this shape even though the union omits the type.
 */
export type DurableAssistantChunk = {
...
export function coalesceReplayEvents(events: readonly SessionEvent[]): SessionEvent[] {
  ...
  let run: { event: DurableAssistantChunk; type: string; parts: string[] } | null = null
  const flush = (): void => {
    if (run === null) return
  ...
  for (const event of events) {
    if ((event as { type: string }).type === 'assistant/chunk') {
      ...
      const chunkType = chunkEvent.data.chunk.type
      if (chunkType === 'text-delta' || chunkType === 'reasoning-delta') {
        if (run !== null && run.type === chunkType) {
          run.parts.push(chunkEvent.data.chunk.text ?? '')
          continue
        }
        flush()
        run = { event: chunkEvent, type: chunkType, parts: [chunkEvent.data.chunk.text ?? ''] }
        continue
      }
    }
    flush()
...
export function extractEntries(sessionId: string, events: readonly SessionEvent[]): TreeEntry[] {
  ...
  for (const event of coalesceReplayEvents(events)) {
    ...
    // `assistant/chunk` rows the union no longer names: synthesize a
    // tentative assistant entry per chunk run (see `tentatives`).
    if ((event as { type: string }).type === 'assistant/chunk') {
      const chunkEvent = event as unknown as DurableAssistantChunk
      ...
      const key = `${chunkEvent.data.turn}:${chunkEvent.data.step}`
      const index = push({
        seq: chunkEvent.seq,
        kind: 'assistant',
        text: preview(text),
