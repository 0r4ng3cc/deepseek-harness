 ...
 *
 * @module @x1a0f3n9/dsh-tui/compat/sessionLog
 */
...
function decodeStorageRecord(value: unknown): SessionEvent[] {
  if (cachedDecoder === undefined) {
    ...
    cachedDecoder = (req('@x1a0f3n9/dsh-session') as { decodeStorageRecord: StorageDecoder }).decodeStorageRecord
  }
...
export function ensureLegacySessionEventTypes(): void {
  ...
  const walk = (anchor: string): void => {
    ...
    try {
      const copy = req.resolve('@x1a0f3n9/dsh-session')
    ...
    try {
      walk(req.resolve('@x1a0f3n9/dsh-session-persistence'))
    } catch {
