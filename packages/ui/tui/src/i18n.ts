...
const dict = {
  ...
  'export-title': { zh: '# dsh 会话导出', en: '# dsh session export' },
  ...
  'system-editor-title': { zh: '自定义系统提示词', en: 'Custom system prompt' },
  'system-editor-hint': { zh: 'Enter 换行 · Ctrl+Enter 保存 · Esc 取消。空内容表示不追加自定义段。', en: 'Enter newline · Ctrl+Enter save · Esc cancel. Empty text means no extra section.' },
  'system-editor-save': { zh: '保存', en: 'Save' },
  'system-editor-cancel': { zh: '取消', en: 'Cancel' },
  'system-saved': { zh: '自定义系统提示词已保存（下一轮请求生效）', en: 'Custom system prompt saved (applies on the next request)' },
  'system-save-failed': { zh: '无法写入 ~/.dsh-tui/system-prompt.txt', en: 'Cannot write ~/.dsh-tui/system-prompt.txt' },
  'system-cleared': { zh: '已清空自定义系统提示词', en: 'Custom system prompt cleared' },
  ...
  'login-logout-hint': { zh: '使用 /provider 管理 DSH 凭据；若来源为 env，请删除对应环境变量并重启 dsh', en: 'Manage DSH credentials with /provider; for env sources, remove the corresponding environment variable and restart dsh' },
  ...
  'restart-starting': { zh: '正在重启 dsh，完成后自动恢复当前会话……', en: 'Restarting dsh. The session resumes when it comes back…' },
  ...
  'logo-drift-newer': {
    zh: 'dsh 引擎为 {{installed}}，比本界面验证过的 {{validated}} 新，可能出现兼容问题；求稳可执行 npm i -g @x1a0f3n9/dsh@{{primary}} 降级，或等待 dsh-tui 适配新版。',
    en: 'The dsh engine ({{installed}}) is newer than the {{validated}} this UI is validated against, so issues are possible; downgrade via npm i -g @x1a0f3n9/dsh@{{primary}} for stability, or wait for a dsh-tui update.',
  },
  'logo-drift-older': {
    zh: 'dsh 引擎为 {{installed}}，低于本界面验证过的 {{validated}}，部分功能可能不可用；建议执行 npm i -g @x1a0f3n9/dsh@{{primary}} 升级。',
    en: 'The dsh engine ({{installed}}) is older than the {{validated}} this UI is validated against; some features may be missing. Upgrade via npm i -g @x1a0f3n9/dsh@{{primary}}.',
  },
  'logo-drift-mixed': {
    zh: '检测到 dsh 引擎多版本混装（{{installed}}），容易出现奇怪问题；建议执行 npm i -g @x1a0f3n9/dsh@{{primary}} 统一版本。',
    en: 'Mixed dsh engine versions detected ({{installed}}), which can cause odd behavior; unify them via npm i -g @x1a0f3n9/dsh@{{primary}}.',
  },
  'logo-drift-broken': {
    zh: 'dsh 引擎版本异常（{{installed}}），本界面验证过 {{validated}}；建议执行 npm i -g @x1a0f3n9/dsh@{{primary}} 重装。',
    en: 'Unexpected dsh engine versions ({{installed}}); this UI is validated against {{validated}}. Reinstall via npm i -g @x1a0f3n9/dsh@{{primary}}.',
  },
  ...
  'settings-fullscreen-restart': { zh: '全屏设置已保存，重启 dsh 后生效', en: 'Fullscreen preference saved — restart dsh to apply' },
  ...
  'provider-protocol-anthropic-desc': { zh: 'Anthropic Messages API（Claude 兼容网关）', en: 'Anthropic Messages API (Claude-compatible gateways)' },
  'provider-protocol-google-desc': { zh: 'Google Generative Language API（Gemini 兼容网关）', en: 'Google Generative Language API (Gemini-compatible gateways)' },
  'provider-protocol-codex-desc': { zh: 'OpenAI Codex Responses API（Codex 兼容网关）', en: 'OpenAI Codex Responses API (Codex-compatible gateways)' },
  ...
  'cmd-desc-config': { zh: '查看 dsh 配置来源' },
  ...
  'cmd-desc-restart': { zh: '重启 dsh 并恢复当前会话' },
  'cmd-desc-exit': { zh: '退出 dsh' },
