...
const dict = {
  ...
  'update-unavailable': { zh: '当前运行方式不支持自动更新（需经 dsh --profile 启动），请在终端执行 dsh plugin --profile <name> update @deepseek-ai/dsh-tui', en: 'Automatic update is unavailable in this launch mode (needs dsh --profile). Run dsh plugin --profile <name> update @deepseek-ai/dsh-tui in a terminal.' },
  ...
  'update-starting': { zh: '正在更新 @deepseek-ai/dsh-tui，完成后会自动重启并恢复当前会话……', en: 'Updating @deepseek-ai/dsh-tui. The TUI will restart and resume this session when finished…' },
