...
function Button(t0: Props) {
  ...
  let ref: Props['ref']
  let style: Except<Styles, 'textWrap'>
  let t1: Props['tabIndex']
  if ($[0] !== t0) {
    ({
      onAction,
      tabIndex: t1,
    ...
    onAction = $[3] as Props['onAction']
    ref = $[4] as Props['ref']
    style = $[5] as Except<Styles, 'textWrap'>
    t1 = $[6] as Props['tabIndex']
  }
  ...
  useEffect(t2 as React.EffectCallback, t3 as React.DependencyList)
  let t4: (e: KeyboardEvent) => void
  if ($[9] !== onAction) {
    t4 = (e: KeyboardEvent) => {
    ...
    t4 = $[10] as (e: KeyboardEvent) => void
  }
  const handleKeyDown = t4
  let t5: (_e: unknown) => void
  if ($[11] !== onAction) {
    t5 = (_e: unknown) => {
      onAction()
    }
    ...
    t5 = $[12] as (_e: unknown) => void
  }
  const handleClick = t5
  let t6: (_e: unknown) => void
  if ($[13] === Symbol.for('react.memo_cache_sentinel')) {
    t6 = (_e_0) => { setIsFocused(true) }
    $[13] = t6
  } else {
    t6 = $[13] as (_e: unknown) => void
  }
  const handleFocus = t6
  let t7: (_e: unknown) => void
  if ($[14] === Symbol.for('react.memo_cache_sentinel')) {
    t7 = (_e_1) => { setIsFocused(false) }
    $[14] = t7
  } else {
    t7 = $[14] as (_e: unknown) => void
  }
  const handleBlur = t7
  let t8: () => void
  ...
  if ($[17] !== children || $[18] !== isActive || $[19] !== isFocused || $[20] !== isHovered) {
    ...
    t10 = typeof children === 'function' ? (children as (state: ButtonState) => React.ReactNode)(state) : children
    $[17] = children
    $[18] = isActive
    $[19] = isFocused
    $[20] = isHovered
    ...
    t10 = $[21] as React.ReactNode
  }
  const content = t10
  let t11: React.ReactNode
  if (
    $[22] !== autoFocus
    || $[23] !== content
    || $[24] !== handleClick
...
function _temp(setter: (value: boolean) => void): void {
  setter(false)
}
