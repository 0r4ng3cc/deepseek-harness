...
export const Ansi = React.memo(function Ansi(t0: Props) {
  ...
  if (typeof children !== 'string') {
    ...
    return t1 as React.ReactNode
  }
  if (children === '') {
    return null
  }
  let t1: React.ReactNode
  let t2: React.ReactNode | symbol
  if ($[3] !== children || $[4] !== dimColor) {
    ...
    bb0: {
      ...
      t1 = spans.map(t3 as (span: Span, i: number) => React.ReactNode)
    }
    $[3] = children
    $[4] = dimColor
    $[5] = t1
    ...
    t1 = $[5] as React.ReactNode
    t2 = $[6] as React.ReactNode | symbol
  }
  if (t2 !== Symbol.for('react.early_return_sentinel')) {
    return t2 as React.ReactNode
  }
  const content = t1
  let t3
