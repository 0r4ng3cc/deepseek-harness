...
declare const inkFiberRootBrand: unique symbol
type InkFiberRoot = { readonly [inkFiberRootBrand]: true }
...
export default class Ink {
  ...
  private readonly container: InkFiberRoot
  ...
  constructor(private readonly options: Options) {
    ...
    // Ported CC build: @types/react-reconciler@0.32.3 declares 11 args with
    // transitionCallbacks, but react-reconciler 0.33.0 source only accepts 10.
    // FiberRoot in that package is `any`, so the local brand keeps the field typed.
    const createContainer = reconciler.createContainer.bind(reconciler) as (
      ...args: unknown[]
    ) => InkFiberRoot
  ...
  render(node: ReactNode): void {
    ...
    const tree = (
      <App
        ...
        onContextMenuAt={this.dispatchContextMenu}
        onHoverAt={this.dispatchHover}
        onWheelAt={this.dispatchWheelAt}
        ...
        onDragTargetAt={this.findDragTargetAt}
        onDragDispatch={this.dispatchDrag}
        onPointerGestureChange={this.setPointerGestureActive}
        onProtocolCandidateChange={this.setProtocolCandidateActive}
        onReleaseTail={this.drainReleaseTail}
        onClickProbe={this.clickProbeAtBatchTail}
        onStdinResume={this.reassertTerminalModes}
        onTerminalFocus={this.handleTerminalFocusProbe}
...
export function drainStdin(stdin: NodeJS.ReadStream = process.stdin): void {
  ...
  const wasRaw = tty.isRaw  // Drain the kernel TTY buffer via a fresh O_NONBLOCK fd. Bounded at 64
