...
describe('Python runtime executable builder CLI', () => {
  it('keeps the single-file dispatcher on the Python packaging surface', () => {
    ...
    expect(cliManifest.dependencies).not.toHaveProperty('@x1a0f3n9/dsh-subprocess-local')
    expect(cliManifest.devDependencies).toHaveProperty('@x1a0f3n9/dsh-subprocess-local')
    expect(runtimeManifest.dependencies).toHaveProperty('@x1a0f3n9/dsh-subprocess-local')
    expect(bootstrap).toContain("import('@x1a0f3n9/dsh/lib/bin.js')")
    expect(bootstrap).toContain('await runCli()')
    expect(bootstrap).toContain("import('@x1a0f3n9/dsh-subprocess-local/runner')")
