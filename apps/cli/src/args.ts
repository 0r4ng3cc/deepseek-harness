...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    // its -h. `xfdsh -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: xfdsh --profile <name> --help)')
    ...
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      ...
      // `xfdsh -h` (no profile to hand it to) must print.
  ...
  if (resolved === undefined) throw new Error('xfdsh: no invocation resolved')
  return resolved
}
