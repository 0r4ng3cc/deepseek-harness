...
| `dsh --profile <name> --from-default-profile <template>` | Create a missing profile from a shipped template, then boot it. |
...
The invoking directory is the default workspace root. The `web`, `headless`, `sdk`, `sdk-minimal`, and `acp` profiles auto-initialize on first use from shipped templates. Create another profile from one of those templates with `--from-default-profile`, or initialize a base-backed profile through `dsh plugin`.
