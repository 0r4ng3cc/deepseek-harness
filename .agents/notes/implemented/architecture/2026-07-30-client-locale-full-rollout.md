...
**Every product-authored UI phrase is translated.** Client fallbacks, design labels, trajectory inspection, accessibility names, and formatter units are dictionary-owned under the [locale-owned copy decision](2026-08-23-locale-owned-client-ui-copy.md). User/model/provider text, externally authored plugin or wire text, and protocol or code tokens remain verbatim data. Wire transport does not make first-party product copy external: the command UI recognizes each built-in Host description by its exact command name and canonical English source before translating it. Framework-free boot markup still runs before the locale service; the localized application replaces its product copy after activation.
