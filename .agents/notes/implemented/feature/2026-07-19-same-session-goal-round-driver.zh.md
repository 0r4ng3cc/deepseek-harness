...
异常结果都不会请求自动重试。之后的人类提示词可以用任何语言要求继续；模型读取 active-but-disarmed 或 blocked 目标并调用目标工具的 resume 动作，记录新修订并重新激活继续执行。持久的 paused 目标依据[用户独占暂停决策](../bug-fix/2026-09-03-user-owned-goal-pause-activation.zh.md)使用 `/goal resume` 或 Web 控件。
...
- **自动重试提供方或持久化错误**——不予采纳，因为重试会消耗资源，需要显式授权；停止阶段与用户独占的 paused 恢复，或模型恢复 blocked／disarmed 目标，更简单也可观察。
