...
- Policy is enforced host-side per request; the client vocabulary never expands what the route allows. Media typing trusts the extension allowlist; byte-signature validation stays with the `read_image` tool that owns it.
