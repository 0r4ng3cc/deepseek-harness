...
- 恢复后的会话等待人类决策；`/goal resume` 是字面命令路径。任何语言的普通提示词都可以为 active-but-disarmed 或 blocked 目标授权模型工具路径，而持久 paused 目标依据[用户独占暂停决策](../bug-fix/2026-09-03-user-owned-goal-pause-activation.zh.md)保留在直接命令路径上。
