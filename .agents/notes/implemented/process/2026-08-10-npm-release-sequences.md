...
The dsh family publishes to the `@x1a0f3n9` scope on npmjs.com. The vendored framework and native packages remain under `@deepseek-ai`; access is per release sequence rather than per scope, and all three current release sequences publish public artifacts ([rationale](2026-08-13-public-vendor-and-native-sequences.md)). No publish path passes `--access`, because one flag cannot serve sequences that disagree and would override the manifest that owns the level.
