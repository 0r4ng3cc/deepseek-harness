...
Access is a property of the package, not of a version. The dsh family is now public under `@x1a0f3n9`, while the vendored and native families remain public under `@deepseek-ai`; future versions keep the access level declared by their own manifests.
...
- **`@deepseek-ai/dsh` is installable by consumers outside the organization.** Its dsh manifests are public, and its vendored and native dependencies are public under their separate `@deepseek-ai` release sequences.
