...
The first-party dsh release family uses the `@x1a0f3n9` scope: the launcher is `@deepseek-ai/dsh`, the workspace root is `@deepseek-ai/dsh-root`, and every publishable package under `packages/*/*` and `apps/*` uses `@deepseek-ai/dsh-*` or an application package name under that scope. The published launcher executable is `xfdsh`, so it can coexist with the official `dsh` command.
...
Consumers of this fork install `@deepseek-ai/dsh` rather than `@deepseek-ai/dsh`. Existing package imports using the former dsh names must be updated when moving to this fork.
